...
<h1 class="d-flex justify-content-center color">Deposit History</h1>
 <?php
session_start();
require 'db.php';
...
$sql = "SELECT amount, time, state FROM deposit WHERE name = '$username'";
...
if ($result && $result->num_rows > 0) {
  ...
  echo '<div class="deposit-item my-3 p-3" style="border-bottom:1px solid #ddd;">';
        echo '<p style="font-size:18px; font-weight:bold; margin:0;">Deposit <span style="float:right;">' . htmlspecialchars($row['amount']) . '$</span></p>';
        echo '<p style="font-size:14px; color:#555; margin:0;">' . htmlspecialchars($row['time']) . ' <span style="float:right;">' . htmlspecialchars($row['state']) . '</span></p>';
        echo '</div>';
